...
import { fetchByCategory } from "./lib/cases";
import { staticFallback } from "./lib/staticCases";
import { CATEGORIES, CATEGORY_LABELS } from "./types";
import type { CaseItem, Category } from "./types";

const imgKV = "/images/KVimg.webp";
const imgImg = "/images/54d68b18c1b63f9c8456cb00cd5868cdb04fe963.webp";
const imgImg1 = "/images/becc429958d279e6672203322f7801c2104c7cb7.webp";
const imgImg2 = "/images/36736eaa0e8deb6e7a161d9b3cd9c3bb7092e592.webp";
const imgContainer = "/images/ce796b1a5e0c977d91cf91edac792b00bc918e1b.webp";
  ...
  return (
    <div className="bg-[#edede9] flex flex-col gap-[10px] w-full overflow-clip relative shrink-0 md:flex-1">
      <div className="h-[240px] md:h-[500px] lg:h-[679px] overflow-clip relative shrink-0 w-full">
        <img alt="" className="absolute inset-0 size-full object-cover pointer-events-none" src={imgImg} loading="lazy" decoding="async" onError={(e) => { e.currentTarget.style.display = 'none'; }} />
        <ServiceCardOverlay
          title="일반도장 시공"
          description={
            <>
...
function Frame15() {
  return (
    <div className="bg-[#edede9] flex flex-col gap-[10px] w-full overflow-clip relative shrink-0 md:flex-1">
      <div className="h-[240px] md:h-[500px] lg:h-[679px] overflow-clip relative shrink-0 w-full">
        <img alt="" className="absolute inset-0 size-full object-cover pointer-events-none" src={imgImg1} loading="lazy" decoding="async" onError={(e) => { e.currentTarget.style.display = 'none'; }} />
        <ServiceCardOverlay
          title="스페셜 도장"
...
function Frame16() {
  return (
    <div className="bg-[#edede9] flex flex-col gap-[10px] w-full overflow-clip relative shrink-0 md:flex-1">
      <div className="h-[240px] md:h-[500px] lg:h-[679px] overflow-clip relative shrink-0 w-full">
        <img alt="" className="absolute inset-0 size-full object-cover pointer-events-none" src={imgImg2} loading="lazy" decoding="async" onError={(e) => { e.currentTarget.style.display = 'none'; }} />
        <ServiceCardOverlay
          title="컬러 컨설팅"
...
function TabList({ activeTab, onTabChange }: { activeTab: TabType; onTabChange: (tab: TabType) => void }) {
  return (
    <div className="relative rounded-[14px] shrink-0 w-full px-4" data-name="Tab List">
      <div className="flex flex-row flex-wrap items-center justify-center gap-[8px] md:gap-[12px] px-[3px] py-[4px] w-full max-w-[900px] mx-auto">
        {CATEGORIES.map((c) => (
          <PrimitiveButton key={c} isActive={activeTab === c} onClick={() => onTabChange(c)}>
            {CATEGORY_LABELS[c]}
          </PrimitiveButton>
        ))}
      </div>
...
function Frame({ items, onItemClick }: { items: CaseItem[]; onItemClick: (i: number) => void }) {
  return (
    <div className="flex flex-row gap-[10px] md:gap-[20px] shrink-0 md:w-[2400px] lg:w-[3476px] md:h-[450px] lg:h-[690px]">
      {items.map((item, i) => (
        <div
          key={item.id}
          className="relative flex-none overflow-hidden w-[80vw] aspect-[4/5] [scroll-snap-align:start] md:[scroll-snap-align:none] md:flex-1 md:w-auto md:aspect-auto md:h-full cursor-pointer"
          onClick={() => onItemClick(i)}
        >
          <img alt={item.title ?? ''} className="absolute inset-0 size-full object-cover pointer-events-none" src={item.thumb_url} loading="lazy" decoding="async" onError={(e) => { e.currentTarget.style.display = 'none'; }} />
        </div>
      ))}
    </div>
  );
...
function Frame26({ items, onItemClick }: { items: CaseItem[]; onItemClick: (i: number) => void }) {
  ...
  return (
    ...
    >
      <Frame items={items} onItemClick={onItemClick} />
    </div>
  );
}
...
function MoreButton({ onClick }: { onClick: () => void }) {
  return (
    <button
      onClick={onClick}
      className="group flex items-center gap-2 border border-[#3b3b3b] rounded-full px-7 py-[10px] text-[#3b3b3b] hover:bg-[#3b3b3b] hover:text-white transition-colors font-['Fahkwang:Regular','Noto_Sans_KR:Regular',sans-serif] text-[14px] md:text-[16px]"
...
function Section1() {
  ...
  useEffect(() => {
    let alive = true;
    fetchByCategory(activeTab, 12)
      .then((rows) => {
        if (!alive) return;
        setItems(rows.length ? rows : staticFallback(activeTab));
      })
      .catch(() => { if (alive) setItems(staticFallback(activeTab)); });
    return () => { alive = false; };
  }, [activeTab]);
  ...
  return (
    <div className="bg-[#edede9] flex flex-col gap-[30px] md:gap-[35px] items-center justify-center overflow-clip pb-0 pt-[60px] md:pt-[100px] relative shrink-0 w-full" data-name="Section3">
      ...
      <Frame26 items={items} onItemClick={setLightboxIndex} />
      <MoreButton onClick={() => navigate(`/cases/${activeTab}`)} />
      {lightboxIndex !== null && (
        <Lightbox
          items={items}
          index={lightboxIndex}
